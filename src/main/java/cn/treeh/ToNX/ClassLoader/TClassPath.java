...
public class TClassPath {
    private static final boolean DEBUG;
    private static final boolean DISABLE_JAR_CHECKING;
    private static final boolean DISABLE_ACC_CHECKING;
    static {
        Properties props = PropsUtil.loadProps("TClassLoader.prop");
        DEBUG = (props.getProperty("sun.misc.TClassPath.debug") != null);
        String p = props.getProperty("sun.misc.TClassPath.disableJarChecking");
        DISABLE_JAR_CHECKING = p != null && (p.equals("true") || p.equals(""));
        p = props.getProperty("jdk.net.TClassPath.disableRestrictedPermissions");
        DISABLE_ACC_CHECKING = p != null && (p.equals("true") || p.equals(""));
    }
    ...
    /**
     * Creates a new TClassPath for the given URLs. The URLs will be
    ...
    public TClassPath(URL[] urls, AccessControlContext acc) {
        this(urls, null, acc);
    }

    /**
     * Constructs a TClassPath from a class path string.
     *
     ...
     */
    TClassPath(String cp, boolean skipEmptyElements) {
     ...
     */
    public TResource getResource(String name, boolean check) {
        if (DEBUG) {
            System.err.println("TClassPath.getResource(\"" + name + "\")");
        }

        Loader loader;
        for (int i = 0; (loader = getLoader(i)) != null; i++) {
            TResource res = loader.getResource(name, check);
            if (res != null) {
                return res;
            }
        }
     ...
     */
    public Enumeration<TResource> getResources(final String name,
                                              final boolean check) {
        return new Enumeration<>() {
            private int index = 0;
            private TResource res = null;
    ...
    private synchronized Loader getLoader(int index) {
        ...
        while (loaders.size() < index + 1) {
            ...
            try {
                ...
                // this TClassPath was given during construction will never
    ...
    private static class Loader implements Closeable {
        ...
        TResource getResource(final String name, boolean check) {
            final URL url;
            try {
                url = new URL(base, ParseUtil.encodePath(name, false));
            } catch (MalformedURLException e) {
         ...
         * resource.
         */
        TResource getResource(final String name) {
            return getResource(name, true);
        }
